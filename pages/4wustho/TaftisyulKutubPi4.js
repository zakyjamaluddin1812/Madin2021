const TaftisyulKutubPi4 = () => {
    const app = document.getElementById('app');
    app.innerHTML = `<div class="kelas px-4" data-aos="fade">
    <h1 class="mb-5">- TAFTISUL KUTUB -</h1>
    <form action="" method="post" id="form" name="form-nilai">
      <input type="hidden" name="mapel" value="Taftisyul Kutub">
      ...
      <div class="d-flex justify-content-between">
        <button onclick="kelas4()" class="btn btn-outline-secondary btn-mapel">KEMBALI</button>
        <div class="pisah"></div>
...
// Get Data
  $.ajax({
    ...
    const kelas1 = data.filter(el => el.kelas == '4' && el.gender === 'p')
    var i = 1
  ...
  // Post Data
  const scriptURL = 'https://script.google.com/macros/s/AKfycbx-Fnavub4tfhHjMPAlH_afwYprDHFryRRWA1f_csW8U03y-LgRCCDSZpC_pTmmWOiCzA/exec'
      ...
      if (result.isConfirmed) {
        ...
        fetch(scriptURL, { method: 'POST', body: new FormData(form)})
          .then(response => {
            ...
            setCookie('TaftisyulKutubPi4', 'sudah', 20)
            kelas4()
          })
          .catch(error => {
            Swal.fire({
