...
// Get Data
  $.ajax({
    ...
    const kelas1 = data.filter(el => el.kelas == '1')
    kelas1.forEach(santri => {
      const baris = document.getElementById('baris');
      const tr = document.createElement('tr')
      tr.innerHTML = `<th scope="row">${data.indexOf(santri)+1}</th>
      ...
      if (result.isConfirmed) {
        ...
        fetch(scriptURL, { method: 'POST', body: new FormData(form)})
          .then(response => {
            ...
            setCookie('TuhfatulAtfal1', 'sudah', 20)
            kelas1()
          })
          .catch(error => {
            Swal.fire({
