const TaftisyulKutubPa1 = () => {
    ...
    <form action="" method="post" id="form" name="form-nilai">
      <input type="hidden" name="mapel" value="Taftisyul Kutub Pa">
...
// Get Data
  $.ajax({
    ...
    const kelas1 = data.filter(el => el.kelas == '1' && el.gender === 'l')
      ...
      if (result.isConfirmed) {
        ...
        fetch(scriptURL, { method: 'POST', body: new FormData(form)})
          .then(response => {
            ...
            setCookie('TaftisyulKutubPa1', 'sudah', 20)
            kelas1()
          })
          .catch(error => {
            Swal.fire({
